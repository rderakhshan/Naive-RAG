...
def query_documents(question, n_results=2):
    """
    Query documents from ChromaDB based on a question.
    """
    results = collection.query(query_texts=question, n_results=n_results)

    # print(results)
    relevant_chunks = [doc for sublist in results["documents"] for doc in sublist]
    # print(f"The first retrived chunk is: {relevant_chunks[0] if relevant_chunks else 'No relevant chunks found'}")
    # print(f"The second retrived chunk is: {relevant_chunks[1] if relevant_chunks else 'No relevant chunks found'}")
...
def generate_response(question, relevant_chunks):
    """
    Generate a response from OpenAI using the question and only and only based on relevant retrived chunks. 
    In case, if did not find anything related in the retrived chunks, it will return a message that it does not know the answer.
    """
